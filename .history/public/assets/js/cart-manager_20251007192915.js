...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildColors() {
    ...
    if (!container) return;
    container.innerHTML = "";
    ["hsl(0, 0%, 100%)", "hsl(37, 100%, 65%)", "hsl(220, 100%, 65%)", "hsl(340, 100%, 65%)", "hsl(126, 61%, 52%)", "#f44336"].forEach(
      (color) => {
        const el = document.createElement("div");
        el.className = "color__link";
        el.style.background = color;
        if (color === "#ffffff") el.classList.add("color-white");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (detailsBtn) {
      ...
      // ✅ FIXED: Show success message directly here
      if (typeof showNotification === "function") {
        showNotification("Product added to cart successfully!");
      } else {
...
if (typeof showNotification === 'undefined') {
  function showNotification(message, type = 'success') {
    ...
    notification.style.cssText = `
      ...
      padding: 12px 20px;
      border-radius: 4px;
      background: ${type === 'success' ? 'var(--first-color)' : 'var(--error-color)'};
      color: white;
      z-index: 10000;
