...
function reattachWishlistEventListeners() {
  ...
  // Add to Cart buttons - Use event delegation for better reliability
...
function openWishlistItemPopup(row) {
  ...
  const product = {
    ...
    quantity: 1,
    fromWishlist: true // Add this flag to identify wishlist items
  };
  ...
  if (typeof window.openPopup === 'function') {
    ...
    // Fallback: try direct popup
    if (openWishlistPopupDirectly(product)) {
      console.log('✅ Using direct popup fallback');
    } else {
      console.log('🔄 Ultimate fallback: adding directly to cart');
      // Ultimate fallback: add directly to cart
...
function removeWishlistItemById(id) {
  ...
  // Refresh the wishlist display if we're on the wishlist page
  ...
  return wishlistItems.length !== initialLength; // Return true if item was removed
}
