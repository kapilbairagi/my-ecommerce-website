...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function openPopup(product) {
    ...
    // Handle wishlist items specifically
    if (product.fromWishlist) {
      handleWishlistItemAdd(product);
    } else {
      handleRegularProductAdd(product);
    }
  ...
  function handleRegularProductAdd(product) {
    ...
    newConfirmBtn.addEventListener("click", function() {
      ...
      console.log('Adding product to cart:', finalProduct);
  ...
  function handleWishlistItemAdd(product) {
    ...
    // Add new event listener specifically for wishlist items
    newConfirmBtn.addEventListener("click", function() {
      if (!selectedColor || !selectedSize) {
        const missingFields = [];
        if (!selectedColor) missingFields.push("color");
      ...
      console.log('Adding wishlist item to cart:', finalProduct);

      // Add to cart using the regular function
      addToCart(finalProduct);
      
      // Remove from wishlist after successful addition to cart
      if (typeof removeWishlistItemById === 'function') {
...
function openPopup(product) {
  ...
  if (qtyInput) qtyInput.value = product.quantity || 1;
...
// Helper function to get product from details page
...
function initializeCheckoutButton() {
    const checkoutBtn = document.querySelector('.cart__total a[href="checkout.html"]');
    const cartItems = JSON.parse(localStorage.getItem('cartItems')) || [];
...
// Update the initializeCart function to include checkout button
function initializeCart() {
    loadCartItems();
    calculateCartTotal();
    initializeCheckoutButton(); // Add this line
    ...
    if (updateCartBtn) {
        updateCartBtn.addEventListener('click', function (e) {
            ...
            updateCartCount();
            updateAllCartItemsInLocalStorage();
            initializeCheckoutButton(); // Update button state
            showNotification('Cart updated successfully!');
        });
    }
    
    updateCartTotalsOnQuantityChange();
}

// Update checkout button state when items are added/removed
...
// Update removeCartItem function to handle checkout button state
function removeCartItem(row) {
    const id = row.dataset.id;
    ...
    setTimeout(() => {
        ...
        checkEmptyCart();
        updateCheckoutButtonState(); // Update checkout button
    }, 300);
}
