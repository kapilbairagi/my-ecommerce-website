...
function initializeCartButtons() {
  ...
  document.querySelectorAll('.cart__btn').forEach(button => {
    button.addEventListener('click', function(e) {
      ...
      // Try multiple ways to access the popup function
      if (typeof window.openPopup === 'function') {
        window.openPopup(productData);
      } else if (typeof openPopup === 'function') {
        openPopup(productData);
      } else {
        // Fallback: check if popup elements exist and manually open
        const popup = document.getElementById('cart-popup');
        if (popup) {
          console.log('Manual popup opening');
          // You might need to manually set up the popup here
          alert('Popup functionality not available. Please check console.');
        } else {
          alert('Add to Cart: Please select color and size on product details page.');
        }
      }
    });
  });
