...
function loadCartItems() {
  ...
  // Color mapping from HSL to readable names
  const colorMap = {
    'hsl(0, 0%, 0%)': 'Black',
    'hsl(37, 100%, 65%)': 'Orange',
    'hsl(220, 100%, 65%)': 'Blue',
    'hsl(340, 100%, 65%)': 'Pink',
    'hsl(126, 61%, 52%)': 'Green',
    'hsl(0, 0%, 100%)': 'White'
  ...
  cartItems.forEach(item => {
    ...
    tr.innerHTML = `
        ...
        <td>
          <span class="table__color" style="background-color:${item.color || '#ccc'};" title="${colorName}"></span>
          <span class="table__color-name">${colorName}</span>
        </td>
