...
function openPopupDirect(product) {
  ...
  if (categoryEl) categoryEl.innerHTML = `Category: <span>${product.category}</span>`;
  ...
  if (confirmBtn) {
    ...
    newConfirmBtn.addEventListener("click", function() {
      const selectedColor = document.querySelector('#popup-colors .color-active')?.style.backgroundColor || 'hsl(0, 0%, 100%)';
      const selectedSize = document.querySelector('#popup-sizes .size-active')?.textContent || 'M';
...
function buildColorOptions() {
  ...
  colors.forEach((color, index) => {
    ...
    el.className = "color__link" + (index === 0 ? " color-active" : "");
...
function buildSizeOptions() {
  ...
  sizes.forEach((size, index) => {
    const el = document.createElement("div");
    el.className = "size__link" + (index === 1 ? " size-active" : "");
    el.textContent = size;
    el.addEventListener("click", () => {
