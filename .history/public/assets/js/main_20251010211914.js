...
document.addEventListener('DOMContentLoaded', () => {
  const dropdownButton = document.getElementById('categoriesDropdownButton');
  const selectedSpan = dropdownButton.querySelector('.categories-dropdown__selected');
  const links = document.querySelectorAll('.categories-dropdown__link');

  links.forEach(link => {
    link.addEventListener('click', (e) => {
      const clickedText = e.target.textContent.trim();
      selectedSpan.textContent = clickedText;
    });
  });
});
