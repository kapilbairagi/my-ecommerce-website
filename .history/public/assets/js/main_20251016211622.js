...
//==================== SCROLL INDICATORS FOR DROPDOWN ====================//
document.addEventListener('DOMContentLoaded', () => {
  const dropdownList = document.querySelector('.categories-dropdown__list');
  const topIndicator = document.querySelector('.top-indicator');
  const bottomIndicator = document.querySelector('.bottom-indicator');

  if (dropdownList && topIndicator && bottomIndicator) {
    const updateIndicators = () => {
      topIndicator.classList.toggle('hidden', dropdownList.scrollTop <= 0);
      bottomIndicator.classList.toggle(
        'hidden',
        dropdownList.scrollTop + dropdownList.clientHeight >= dropdownList.scrollHeight
      );
    };

    dropdownList.addEventListener('scroll', updateIndicators);
    window.addEventListener('resize', updateIndicators);
    updateIndicators();

    topIndicator.addEventListener('click', () => {
      dropdownList.scrollBy({ top: -80, behavior: 'smooth' });
    });
