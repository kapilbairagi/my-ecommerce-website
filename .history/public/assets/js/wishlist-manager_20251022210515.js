...
function reattachWishlistEventListeners() {
  ...
  // Remove existing event listeners and add new ones for Add to Cart buttons
...
function openWishlistItemPopup(row) {
  ...
  const product = {
    id: id, // This is important - it's the wishlist item ID
    title: title,
    category: category,
    price: price,
  ...
  // Use the global openPopup function
  if (typeof window.openPopup === 'function') {
    console.log('Calling window.openPopup for wishlist item');
...
// Add this debug function at the top of wishlist-manager.js
