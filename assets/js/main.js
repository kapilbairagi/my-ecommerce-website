...
function initializeDetailsPage() {
    ...
    // Initialize color and size selection FIRST
    initializeColorSizeSelection();
    
    // Then initialize buttons
...
function initializeColorSizeSelection() {
  ...
  // Color selector - ENHANCED VERSION
  const colorLinks = document.querySelectorAll(".color__link");
  console.log('Color links found:', colorLinks.length);
  ...
  colorLinks.forEach(color => {
    ...
    newColor.addEventListener("click", function(e) {
      e.preventDefault();
      e.stopPropagation();
      console.log('Color clicked:', this.style.backgroundColor);
      
      // Remove active class from all colors in this section
      const allColors = this.closest('.color__list').querySelectorAll('.color__link');
      allColors.forEach(c => {
  ...
  // Size selector - ENHANCED VERSION
  const sizeLinks = document.querySelectorAll(".size__link");
  console.log('Size links found:', sizeLinks.length);
  ...
  sizeLinks.forEach(link => {
    ...
    newLink.addEventListener("click", function(e) {
      ...
      // Remove active class from all sizes in this section
      const allSizes = this.closest('.size__list').querySelectorAll('.size__link');
      allSizes.forEach(l => {
        l.classList.remove("size-active");
      });
...
function initializeDetailsPageButtons() {
  console.log('Initializing details page buttons');
  
  // Fix for details page Add to Cart button
  const detailsCartBtn = document.querySelector('.details__cart-btn');
  if (detailsCartBtn) {
    console.log('Found details cart button');
    
    // Remove any existing event listeners by cloning
    const newCartBtn = detailsCartBtn.cloneNode(true);
    detailsCartBtn.parentNode.replaceChild(newCartBtn, detailsCartBtn);
    
    newCartBtn.addEventListener('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      console.log('Details page cart button clicked');
      
      const title = document.querySelector('.details__title')?.textContent.trim() || 'Pattern Shirt';
      const priceText = document.querySelector('.new__price')?.textContent.trim() || '$49.99';
      const price = parseFloat(priceText.replace(/[^\d.]/g, ''));
      const img = document.querySelector('.details__img')?.src || 'assets/img/product-8-1.jpg';
      const category = document.querySelector('.details__category')?.textContent.trim() || 'Clothing';
      ...
      const quantity = parseInt(document.querySelector('.details .quantity')?.value || '1', 10);

      console.log('Selected color:', selectedColor);
      console.log('Selected size:', selectedSize);

      if (!selectedColor || !selectedSize) {
        const message = 'Please select color and size before adding to cart!';
      ...
      console.log('Adding to cart from details:', product);
  ...
  // Wishlist button
  const wishlistBtn = document.querySelector('.details__action-btn[aria-label="Add To Wishlist"]');
  if (wishlistBtn) {
    console.log('Found details wishlist button');
...
// Helper function for details page
function initializeDetailsCategory() {
  // This function can be used for any details page category initialization
  console.log('Initializing details category');
}